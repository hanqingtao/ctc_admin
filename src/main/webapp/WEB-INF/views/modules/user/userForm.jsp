...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/user/user/">用户列表</a></li>
		<li class="active"><a href="${ctx}/user/user/form?id=${user.id}">用户<shiro:hasPermission name="user:user:edit">${not empty user.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="user:user:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/user/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="degree" htmlEscape="false" maxlength="1" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
