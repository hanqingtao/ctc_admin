...
<body>
	...
	<form:form id="inputForm" modelAttribute="category" action="${ctx}/cms/category/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <!--<sys:treeselect id="category" name="parent.id" value="${category.parent.id}" labelName="parent.name" labelValue="${category.parent.name}"
					title="栏目" url="/cms/category/treeData" extId="${category.id}" cssClass="required"/>--> 顶级栏目
			</div>
		</div>
		<!-- 
		<div class="control-group">
		...
		-->
		<form:input path="module" type="hidden" value="article" />
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">在首页中显示:</label>
			<div class="controls">
				...
				<span class="help-inline">是否在首页中显示该栏目</span >&nbsp;&nbsp;<span style="color:red;">说明：在首页中最多显示4个栏目,在更多新闻和日常监督栏目下最多显示8个小栏目</span>
			</div>
		</div>
