...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/pedlar/pedlar/">商家信息列表</a></li>
		<li class="active"><a href="${ctx}/pedlar/pedlar/form?id=${pedlar.id}">商家信息<shiro:hasPermission name="pedlar:pedlar:edit">${not empty pedlar.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="pedlar:pedlar:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="pedlar" action="${ctx}/pedlar/pedlar/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">集市(默认)：</label>
			<div class="controls">
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="auditBy" name="auditBy.id" value="${pedlar.auditBy.id}" labelName="" labelValue="${pedlar.auditBy.name}"
					title="用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input name="auditDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${pedlar.auditDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
