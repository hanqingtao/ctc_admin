...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/consumer/consumer/">用户信息列表</a></li>
		<li class="active"><a href="${ctx}/consumer/consumer/form?id=${consumer.id}">用户信息<shiro:hasPermission name="consumer:consumer:edit">${not empty consumer.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="consumer:consumer:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="consumer" action="${ctx}/consumer/consumer/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="area" name="area.id" value="${consumer.area.id}" labelName="area.name" labelValue="${consumer.area.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:radiobuttons path="status" items="${fns:getDictList('consumer_status')}" itemLabel="label" itemValue="value" htmlEscape="false" class=""/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="consumer:consumer:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
